...
type MeetReadyForCheckinContext = {
  meetName: string;
  meetDate: Date;
  location: string | null;
  todayUrl: string;
...
function isAttendingStatus(status?: string | null) {
...
export function buildMeetReadyForCheckinContent(
  context: MeetReadyForCheckinContext,
): MeetReadyContent {
  const childLine = context.childNames.length > 0
    ? `Coming wrestlers: ${formatList(context.childNames)}.`
    : "Your linked wrestler is marked as coming.";
  const locationLine = context.location ? `Location: ${context.location}.` : "Location: To be announced.";

  return {
    emailSubject: `${context.meetName} is ready for check-in`,
    emailText: [
      `Hi ${context.recipientName},`,
      "",
...
export function buildMeetPublishedContent(
  ...
  const childLine = context.childNames.length > 0
    ? `Published wrestlers: ${formatList(context.childNames)}.`
    : "Your wrestler's published bouts are now available.";

  return {
    emailSubject: `${context.meetName} has been published`,
    emailText: [
      `Hi ${context.recipientName},`,
      "",
      ...
      `Meet date: ${formatMeetDate(context.meetDate)}.`,
      childLine,
      "Bout numbers and opponents are now available on My Wrestlers and Today.",
      "",
      `My Wrestlers: ${context.myWrestlersUrl}`,
      `Today: ${context.todayUrl}`,
    ].join("\n"),
...
async function getMeetPublishedRecipients(meetId: string, teamIds: string[]): Promise<MeetReadyRecipient[]> {
  ...
  const wrestlerIdsWithBouts = new Set<string>();
  for (const bout of bouts) {
    wrestlerIdsWithBouts.add(bout.redId);
    wrestlerIdsWithBouts.add(bout.greenId);
  }
  if (wrestlerIdsWithBouts.size === 0) {
    return [];
  }
...
export async function notifyMeetReadyForCheckin(
  meetId: string,
  options: { origin?: string | null } = {},
): Promise<MeetReadyForAttendanceSummary> {
  const transport = resolveNotificationTransport();
  ...
  if (!meet || meet.deletedAt || normalizeMeetPhase(meet.status) !== "READY_FOR_CHECKIN") {
    return buildEmptyMeetReadySummary(transport);
  }

  // This event should only be sent the first time a meet reaches Check-in.
  // Reopening as Draft and returning to Check-in restores workflow state, but
  // should not notify parents again.
  const claimed = await claimMeetEventDispatch("meet_ready_for_checkin", meet.id);
  if (!claimed) {
    const summary = buildEmptyMeetReadySummary(transport);
    summary.skipped = 1;
    return summary;
  ...
  const myWrestlersUrl = `${baseUrl}/parent`;
  const teamIds = meet.meetTeams.map((entry) => entry.teamId);
  const recipients = await getMeetReadyForCheckinRecipients(meet.id, teamIds);
  const teamSummaryMap = new Map<string, NotificationTeamSummary>();
  for (const team of meet.meetTeams) {
    teamSummaryMap.set(team.teamId, {
      teamId: team.teamId,
...
export async function notifyMeetPublished(
  ...
  if (!meet || meet.deletedAt || normalizeMeetPhase(meet.status) !== "PUBLISHED") {
    return buildEmptyMeetReadySummary(transport);
  }
